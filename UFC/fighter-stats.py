...
def fighter_get():
    ...
    fighter_df.to_csv('output/fighter_df.csv', index=False)
    return fighter_df
...
def bio_get(fighter_df, base_url):
    results = []
    for i in range(len(fighter_df['Link'])):
        sub_result = []
        name = fighter_df['Name'][i]

        print('Grabbing bio for ' + name)

        try:
            page = fox_request(base_url + fighter_df['Link'][i] + '-stats')
            table = page.find_all('tbody')[1]  # 1st table is the fighter info, 2nd is event info

            for row in table.find_all('tr'):
                result = [row.find_all('td')[1].text.strip()]
                sub_result.extend(result)

            results.append([name] + sub_result)

        except IndexError:  # some listed pages lead to 404 errors
            print(name + ' returns 404 error')
            continue

    bio_df = pd.DataFrame(columns=['Fighter', 'DoB', 'Height_Weight',
                                   'From', 'Reach', 'Style', 'Trains'])

    bio_df = bio_df.append(pd.DataFrame(results, columns=bio_df.columns))
    bio_df.to_csv('output/bio_df.csv', index=False)
    return bio_df
...
def bio_transform(bio_df):
    ...
    for i in range(len(origin)):
        try:
            city = origin[i].split(',')[0]
            country = origin[i].split(',')[1][1:]
    ...
    bio_df = bio_df[['Name', 'DoB', 'Age', 'Height', 'Weight', 'BMI', 'Reach',
                     'HR_Ratio', 'Style', 'City', 'Country', 'From', 'Trains']]

    bio_df.to_csv('output/bio_df-test.csv', index=False)
    return bio_df

# Gets all striking data for each fighter in fighter df, broken down by time unit and self/opponent
    ...
    for i in range(len(fighter_df['Link'])):
        ...
        try:  # IndexError occurs when trying to find 'tbody' on a 404 page
            for strike_time in range(4):  # 4 striking times (0 - total, 1 - per fight, 2 - per round, 3 - per minute)
                page = fox_request(base_url + fighter_df['Link'][i] + '-stats?time=' + str(strike_time))
...
def df_merge(fighter_df, bio_df, striking_df):

    ufc_df = fighter_df.merge(bio_df, left_on='Name', right_on='Name')
    ufc_df = ufc_df.merge(striking_df, left_on='Name', right_on='Name')
    ufc_df.to_csv('output/UFC_dataset.csv', index=False)
    return ufc_df

# fighter_df = fighter_get()
# bio_df = bio_get(fighter_df, fox_fighter_url)
# striking_df = striking_get(fighter_df, fox_fighter_url)
